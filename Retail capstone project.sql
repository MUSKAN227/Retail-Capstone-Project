...
select shipping_type,Round(avg(purchase_amount),2) as avg_pa
from customer
group by shipping_type
having shipping_type in('Standard','Express')

--5. DO subscriber customers spend money more? 
--compare average spend and total revenue between subscribers and non-subscribers.
select count(customer_id) as total_customers, subscription_status,Round(avg(purchase_amount),2) as avg_spend , sum(purchase_amount) as total_revenue,subscription_status
from customer
group by subscription_status
having subscription_status in('Yes','No')
